...
!function(){
    /**
     * JSHON web test compiler. Beta version.
     */
    var markupPattern = /<!--[\s\S]*?-->|<(\/|\s?)\s*([a-zA-Z][-.:0-9_a-zA-Z]*)((?:\s+[^>]*?(?:(?:'[^']*')|(?:"[^"]*"))?)*)\s*(\/?)>/g,
        ...
        function translate(html){
            ...
            var fests = getFestComponents(html.replace(/\/\/<(.*?)\/\/>/gs,"")),i;
        ...
        //Set `compileScripts()` method on JSHON.
        Object.defineProperty(JSHON,"compileScripts",{
            writable:false,
            value:function(){
                var jshonScripts = document.getElementsByTagName("script");
                jshonScripts = jshonScripts? jshonScripts = Object.values(jshonScripts).filter(function(el,i){
                    return el.attributes&&el.attributes.type&&(el.attributes.type.value+"").toLowerCase()=="jshon";
                }):[];
    
                var parsedScripts =jshonScripts.length?document.createDocumentFragment():null;
