 ...
 */
function newPage({callback,data,htmlEntryPath,renderId,jsEntryPath,jsSSREntryPath,inserts}){
            ...
            from:function JSHONInternalImports(src,server_alternate_src){
                ...
                if(!JSH.exports[server_alternate_src]){
                    if(server_alternate_src.startsWith("/")){
                        ...
                        require(server_alternate_src.replace("/",path.join(jserve.srcDirectory,"/ssr/")))(JSH);
                        dependencyModulePaths.push({
                            internal:server_alternate_src.replace("/module/",path.join(jserve.srcDirectory,"/ssr/inline_modules/")),
                            external:src,
                ...
                if(awaitingTasks.length<=0){
                    ...
                    var ssrHtml = require(htmlEntryPath)({
                        ...
                        render:function(){
                            ...
                            return JSH.ui.render(renderingPage.ref,renderingPage.args)[symbolIdentifier].value+inline_dependencies;
               ...
               if(awaitingTasks.length<=0){
                ...
                var ssrHtml = require(htmlEntryPath)({
                    ...
                    render:function(){
                        ...
                        inline_dependencies+=`};},writable:false,configurable:false});!${require(jsSSREntryPath).toString()}()}();</script>`
                        return JSH.ui.render(pageRef,args)[symbolIdentifier].value+inline_dependencies;
                    }
                },escape,data,inserts);
                var cb = callback;
                renderId[symbol]();
